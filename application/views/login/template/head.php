<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>PT. RTR | <?= $title; ?></title>

    <!-- Google Font: Source Sans Pro -->
    <link rel="stylesheet" href="https://fonts.googleapis.com/css?family=Source+Sans+Pro:300,400,400i,700&display=fallback">
    <!-- Font Awesome -->
    <link rel="stylesheet" href="<?= base_url(); ?>assets/admin32/plugins/fontawesome-free/css/all.min.css">
    <!-- icheck bootstrap -->
    <link rel="stylesheet" href="<?= base_url(); ?>assets/admin32/plugins/icheck-bootstrap/icheck-bootstrap.min.css">
    <!-- Theme style -->
    <link rel="stylesheet" href="<?= base_url(); ?>assets/admin32/dist/css/adminlte.min.css">
    <style>
    body {
      font-family: "Open Sans", sans-serif;
      height: 100vh;
      /* background: url("<?= base_url(''); ?>assets/style/") 50% fixed; */
      /* background-size: cover; */
    }

    .wrapper {
      display: flex;
      align-items: center;
      flex-direction: column;
      justify-content: center;
      width: 100%;
      height: 100%;
      padding: 50px;
      background: rgba(28,49,108,0.8);
      text-align: center;
    }

    .auth-bg {
      background-image: url("<?= base_url(''); ?>assets/style/DA 8197 JH - Belakang.jpg");
      background-size: cover;
      background-repeat: no-repeat;
      background-position: center;
      /* height: 100%;
      width: 100%; */
      /* display: table; */
       vertical-align: middle;
    }


    
  </style>
</head>